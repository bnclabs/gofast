...
func TestUnmessage(t *testing.T) {
	...
	// read tag and its payload
	payload := []byte{
		216, 37, 191, 24, 39, 2, 24, 40, 80,
		159, 102, 99, 108, 105, 101, 110, 116, 1, 26, 0, 160, 0, 0, 96, 255,
		255}
...
func BenchmarkReadtagp(b *testing.B) {
	// read tag and its payload
	payload := []byte{
		216, 37, 191, 24, 39, 2, 24, 40, 81,
		159, 103, 99, 108, 105, 101, 110, 116, 1, 26, 0, 160, 0, 0, 96, 255,
		255}
	for i := 0; i < b.N; i++ {
		readtp(payload)
	}
