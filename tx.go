...
func (t *Transport) framepkt(msg Message, stream *Stream, ping []byte) (n int) {
	...
	n += mapStart(ping[n:])
	n += tag2cbor(tagID, ping[n:])          // hdr-tagId
	n += valuint642cbor(msg.ID(), ping[n:]) // value
	n += tag2cbor(tagData, ping[n:])        // hdr-tagData
